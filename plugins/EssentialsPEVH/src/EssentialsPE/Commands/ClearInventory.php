...
class ClearInventory extends BaseCommand{
    ...
    public function execute(CommandSender $sender, $alias, array $args): bool{
        ...
        if(($gm = $player->getGamemode()) === 1 || $gm === 3){
            $sender->sendMessage(TextFormat::RED . "[Lỗi] " . (isset($args[0]) ? $player->getDisplayName() . "vì" : "bạn đang") . " ở " . $this->getAPI()->getServer()->getGamemodeString($gm) . " chế độ");
            return false;
        }
        $player->getInventory()->clearAll();
        $player->sendMessage(TextFormat::AQUA . "Đã xóa túi đồ của bạn");
        if($player !== $sender){
            $sender->sendMessage(TextFormat::AQUA . $player->getDisplayName() . (substr($player->getDisplayName(), -1, 1) === "s" ? "'" : "'s") . " Túi đồ đã bị xóa");
        }
        return true;
    }
}
