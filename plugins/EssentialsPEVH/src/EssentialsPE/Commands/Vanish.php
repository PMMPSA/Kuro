...
class Vanish extends BaseCommand{
    ...
    public function execute(CommandSender $sender, $alias, array $args): bool{
        ...
        $player->sendMessage(TextFormat::GRAY . "Bạn đang " . ($s = $this->getAPI()->isVanished($player) ? "tàng hình" : "visible"));
        if($player !== $sender){
            $sender->sendMessage(TextFormat::GRAY .  $player->getDisplayName() . " đang $s");
        }
        return true;
    }
}
