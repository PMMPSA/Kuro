...
class WpProtectMgr extends BaseWp implements Listener {
	...
	public function onSCommand(CommandSender $c,Command $cc,$scmd,$world,array $args) {
		switch ($scmd) {
				...
				foreach ($args as $i) {
					...
					if (!$player) {
						...
						if ($player == null || !$player->hasPlayedBefore()) {
							$c->sendMessage(mc::_("§a[KuroMCPE] %1%: not found",$i));
							continue;
						}
					}
					...
					if ($player instanceof Player)
						$player->sendMessage(mc::_("§a[KuroMCPE] You have been added to\n[WP] %1%'s auth list",$world));
				}
				return true;
			case "rm":
				if (!count($args)) return false;
				...
				foreach ($args as $i) {
					...
					if ($this->owner->authCheck($world,$iusr)) {
						...
						$player = $this->owner->getServer()->getPlayer($i);
						if ($player) {
							$player->sendMessage(mc::_("§a[KuroMCPE] You have been removed from\n[WP] %1%'s auth list", $world));
						}
					} else {
			...
			case "unlock":
				...
				$this->owner->getServer()->broadcastMessage(mc::_("§a[KuroMCPE] %1% Đã được mở ra",$world));
				return true;
			case "lock":
				if (count($args)) return false;
				$this->owner->setCfg($world,"protect",$scmd);
				$this->owner->getServer()->broadcastMessage(mc::_("§a[KuroMCPE] %1% Đã được khóa lại",$world));
				return true;
			case "protect":
				if (count($args)) return false;
				$this->owner->setCfg($world,"protect",$scmd);
				$this->owner->getServer()->broadcastMessage(mc::_("§a[KuroMCPE] %1% Đã được bảo vệ",$world));
				return true;
		}
		return false;
	}
	...
	public function onBlockBreak(BlockBreakEvent $ev){
		...
		$this->owner->msg($pl,mc::_("§cBạn không được phép làm điều đó ở đây"));
		$ev->setCancelled();
	}
