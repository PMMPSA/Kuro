...
class WpPvpMgr extends BaseWp implements Listener {
	...
	public function onSCommand(CommandSender $c,Command $cc,$scmd,$world,array $args) {
		...
		switch (substr(strtolower($args[0]),0,2)) {
			case "sp":
				...
				$this->owner->getServer()->broadcastMessage(TextFormat::YELLOW.mc::_("§a[KuroMCPE] Không cho phép PVP tại %1%'s spawn",$world));
				break;
			case "on":
			case "tr":
				$this->owner->unsetCfg($world,"pvp");
				$this->owner->getServer()->broadcastMessage(TextFormat::RED.mc::_("§a[KuroMCPE] PvP được cho phép tại %1%",$world));
				break;
			case "of":
			case "fa":
				$this->owner->setCfg($world,"pvp",false);
				$this->owner->getServer()->broadcastMessage(TextFormat::GREEN.mc::_("§a[KuroMCPE] Không cho phép PVP tại %1%",$world));
				break;
			default:
				return false;
		}
	...
	public function onPvP(EntityDamageEvent $ev) {
		...
		$this->owner->msg($ev->getDamager(),mc::_("§cBạn không được phép làm điều đó tại đây"));
		$ev->setCancelled();
	}
}
